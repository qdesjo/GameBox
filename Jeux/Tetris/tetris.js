...
function detectCollision() {
  //Détection d'une colision avec une pièce en dessous
  getHitInfo();
  if (
...
function checkRows(row, imgDataArray) {
  //Checker si elles sont remplies
  for (var i = 0; i < imgDataArray[row].length; i++) {
    if (
      ...
      return false; //Non remplies
    }
  }
  return true; //Remplies
}
...
function resetTetro() {
  //Après que tout soit mis en place
  tetro.position.y = 0; //Déplacer sur y
  currTetro = randomizeTetro(); //Prendre une nouvelle pièce
  tetro.position.x = (W - 48) / 2; //Déplacer sur x
}

function gameOver() {
  //Faire un message "Game over"
  ...
  clearInterval(game); //Fin de la partie
}

function checkGameOver() {
  //Checker si le joueur à perdu
  getHitInfo();
  if (
    tetro.position.y + currTetro.image.height == H ||
    currTetro.hit1.data[0] != 0 ||
...
function moveX() {
  //Moves the player piece left or right
...
function dropDownTetro() {
  //Drops the tetro piece faster
  while (tetro.position.y != 0) {
    //While the piece didnt hit the bottom
...
function storeKey(ev) {
  //Stores what keys where pressed to make the player piece move later
  arrows = ev.which || ev.keyCode;

  switch (
    arrows //In case the arrow is
  ) {
    case 32: //Down
